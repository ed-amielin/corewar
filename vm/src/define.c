...
/*   Updated: 2019/10/29 19:43:40 by eamielin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	define_flags(t_data *data, char **argv)
{
	...
	{
		((data->count++) && (data->tmp = ft_atoi(argv[data->count++])));
		(argv[data->count] && argv[data->count][0] == '-') ?
		err_massage("After n must be player") : 0;
		(data->tmp > MAX_PLAYERS) ? err_massage("Invalid order") : 0;
		(data->tmp < 1) ? err_massage("Invalid order") : 0;
	}
...
void	define_argc(t_data *data, int argc, char **argv)
{
	...
	{
		data->tmp = 0;
		while (argv[data->count] && argv[data->count][0] == '-')
